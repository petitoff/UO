...
for (i in 1:Powt) {
  ...
  while (zap_pola < 9) {
    ...
    S2 <- sum(rowSums(Pole) == -3) + sum(colSums(Pole) == -3) +
      ...
      sum(sum(diag(apply(Pole, 2, rev))) == -3)
    if (S1 + S2 > 0)
      break # jeśli pojawił się układ potrójny,
    # to przerwij partię
    gracz <- -gracz    # ruch otrzymuje przeciwnik
  }
  #---------------------------------------------------------
  if (gracz == 1)
    gr1 <- gr1 + 1 # jeśli wygrał gracz 1, zwiększ
  # jego licznik
...
# a) Zwycięstwo gracza rozpoczynającego partię
cat("a) Szansa na zwycięstwo gracza rozpoczynającego partię:",
    round(prawdop_wygrania_gr1 * 100, 2), "%\n")

# b) Zakończenie partii w ruchach
cat("b) Histogram liczby ruchów:\n")

# c) Zaznaczenie wszystkich pól w partii
ilosc_partii_zaznaczonych_pól <- sum(zaj_pola == 9)
cat("c) Ilość partii zaznaczonych wszystkich pól:",
    ilosc_partii_zaznaczonych_pól, "\n")
